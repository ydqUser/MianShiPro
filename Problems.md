...
- [业务问题](#业务问题)
    - [如何设计一个接口](#如何设计一个接口)
        - [常规数据有效性校验](#常规数据有效性校验)
        - [幂等性：对接口的多次调用所产生的结果和调用一次是一致的，数据发生改变则需要做幂等，本质上是分布式锁的问题](#幂等性对接口的多次调用所产生的结果和调用一次是一致的数据发生改变则需要做幂等本质上是分布式锁的问题)
        - [安全性](#安全性)
        - [限流机制](#限流机制)
    - [消息队列问题](#消息队列问题)
        - [如何保证消息队列的高可用](#如何保证消息队列的高可用)
        - [如何保证消息不重复消费（一般在Consumer端控制）](#如何保证消息不重复消费一般在consumer端控制)
        - [如何解决消息丢失问题](#如何解决消息丢失问题)
        - [如何保证消息的顺序性](#如何保证消息的顺序性)
        - [如何解决消息堆积问题](#如何解决消息堆积问题)
    - [缓存问题](#缓存问题)
        - [缓存余数据库双写不一致--数据一致性问题](#缓存余数据库双写不一致--数据一致性问题)
        - [缓存雪崩](#缓存雪崩)
        - [缓存穿透](#缓存穿透)
        - [缓存击穿](#缓存击穿)
        - [缓存并发竞争](#缓存并发竞争)
        - [redis](#redis)
    - [分库分表](#分库分表)
        - [为什么要分库分表（高并发系统，数据库层面应该如何设计）？](#为什么要分库分表高并发系统数据库层面应该如何设计)
        - [分库分表中间件](#分库分表中间件)
        - [具体是水平拆分还是垂直拆分](#具体是水平拆分还是垂直拆分)
        - [分库分表方式](#分库分表方式)
        - [唯一id](#唯一id)
                - [判断单链表是否有环以及环的入口检测](#判断单链表是否有环以及环的入口检测)
        - [如何在接口中优雅引入重试机制](#如何在接口中优雅引入重试机制)
